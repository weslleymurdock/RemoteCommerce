...
var logDirectory = Path.Combine(
    ...
    "App_Data",
    "logs");
builder.Logging.AddRemoteCommerceFileLogging(
    builder.Services,
    logDirectory);
...
builder.Services.AddScoped<IMediaStorageProvider>(services =>
    services.GetRequiredService<MediaStorageProviderResolver>().Resolve());
            ...
            OnChallenge = context =>
            {
                    ...
                    !isFramework)
                {
                    context.Response.Redirect(
                        $"/login?returnUrl={Uri.EscapeDataString(context.Request.PathBase + context.Request.Path + context.Request.QueryString)}");
            ...
            OnTokenValidated = async context =>
            {
                ...
                if (user is null ||
                    ...
                    !string.Equals(
                        ...
                        StringComparison.Ordinal))
                {
                    context.Fail("The authentication token has been invalidated.");
                }
            }
        };
    });
...
builder.Services.AddScoped<ILocalizationResourceService>(serviceProvider =>
    serviceProvider.GetRequiredService<LocalizationResourceService>());
...
if (!app.Environment.IsProduction())
{
    app.MapScalarApiReference(
        ...
        configuration => configuration
            .WithTitle(
                $"[{app.Environment.EnvironmentName}] RemoteCommerce API Reference")
