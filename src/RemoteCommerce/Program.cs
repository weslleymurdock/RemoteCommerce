...
builder.Services.AddAuthentication(JwtBearerDefaults.AuthenticationScheme)
    .AddJwtBearer(options =>
    {
        ...
        var issuer = jwt["Issuer"] ?? "RemoteCommerce";
        var audience = jwt["Audience"] ?? "RemoteCommerce";
        ...
        options.Events = new JwtBearerEvents
        {
            ...
            OnChallenge = context =>
            {
                var isApiRequest = context.Request.Path.StartsWithSegments(
                    "/api",
                    StringComparison.OrdinalIgnoreCase);
                var isBlazorRequest = context.Request.Path.StartsWithSegments(
                    "/_blazor",
                    StringComparison.OrdinalIgnoreCase);
                var isFrameworkRequest = context.Request.Path.StartsWithSegments(
                    "/_framework",
                    StringComparison.OrdinalIgnoreCase);
                    ...
                    && !isFrameworkRequest)
                {
                    ...
                    var location = $"/login?returnUrl={Uri.EscapeDataString(returnUrl)}";
            ...
            OnTokenValidated = async context =>
            {
                ...
                {
                    context.Fail("The authentication token is not an access token.");
                    return;
                }
