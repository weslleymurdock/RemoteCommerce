...
builder.Services.AddAuthentication(JwtBearerDefaults.AuthenticationScheme).AddJwtBearer(options =>
{
    ...
    options.Events = new JwtBearerEvents
    {
        ...
        OnChallenge = context => { var isFramework = context.Request.Path.StartsWithSegments("/_blazor") || context.Request.Path.StartsWithSegments("/_framework"); if (!context.Response.HasStarted && !context.Request.Path.StartsWithSegments("/api") && !isFramework) { context.Response.Redirect($"/login?returnUrl={Uri.EscapeDataString(context.Request.PathBase + context.Request.Path + context.Request.QueryString)}"); context.HandleResponse(); } return Task.CompletedTask; },
        OnTokenValidated = async context => { var tokenType = context.Principal?.FindFirst("token_type")?.Value; var subject = context.Principal?.FindFirstValue(JwtRegisteredClaimNames.Sub) ?? context.Principal?.FindFirstValue(ClaimTypes.NameIdentifier); var securityStamp = context.Principal?.FindFirst("security_stamp")?.Value; if (!string.Equals(tokenType, JwtOptions.AccessTokenType, StringComparison.Ordinal) || subject is null || securityStamp is null) { context.Fail("The authentication token is invalid."); return; } var userManager = context.HttpContext.RequestServices.GetRequiredService<UserManager<ApplicationUser>>(); var user = await userManager.FindByIdAsync(subject); if (user is null || user.IsDisabled || !string.Equals(user.SecurityStamp, securityStamp, StringComparison.Ordinal)) context.Fail("The authentication token has been invalidated."); }
    };
});
builder.Services.AddAuthorizationBuilder().AddPolicy(AuthorizationPolicies.Administrator, policy => policy.RequireRole("Administrator")).AddPolicy(AuthorizationPolicies.ManageConfiguration, policy => policy.RequireAssertion(context => context.User.IsInRole("Administrator") || context.User.HasClaim("permission", AuthorizationPolicies.ManageConfiguration))).AddPolicy(AuthorizationPolicies.ManageUsers, policy => policy.RequireAssertion(context => context.User.IsInRole("Administrator") || context.User.HasClaim("permission", AuthorizationPolicies.ManageUsers))).AddPolicy(AuthorizationPolicies.ManageLocalization, policy => policy.RequireAssertion(context => context.User.IsInRole("Administrator") || context.User.HasClaim("permission", AuthorizationPolicies.ManageLocalization))).AddPolicy(AuthorizationPolicies.ManagePlugins, policy => policy.RequireAssertion(context => context.User.IsInRole("Administrator") || context.User.HasClaim("permission", AuthorizationPolicies.ManagePlugins)));
...
builder.Services.AddSingleton<IThemeProvider, DefaultThemeProvider>();
builder.Services.AddSingleton<IMenuContributor, CoreMenuContributor>();
builder.Services.AddSingleton<IMenuProvider, AdminMenuProvider>();
