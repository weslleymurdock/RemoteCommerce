...
/// <summary>Installs trusted plugin NuGet packages and persists their activation metadata for the next host restart.</summary>
...
{
    /// <summary>Installs a plugin package and persists its active installation state.</summary>
    ...
    public async Task<PluginManifest> InstallAsync(string packagePath, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (await db.PluginInstallations.AnyAsync(x => x.PluginId == manifest.Id, cancellationToken)) throw new InvalidOperationException($"Plugin '{manifest.Id}' is already installed.");
