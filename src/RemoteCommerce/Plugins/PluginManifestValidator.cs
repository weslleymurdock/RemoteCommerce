...
public sealed class PluginManifestValidator : IPluginManifestValidator
{
    ...
    public IReadOnlyList<PluginValidationIssue> Validate(PluginManifest manifest)
    {
        ...
        if (!string.IsNullOrWhiteSpace(manifest.EntryAssembly) && !manifest.EntryAssembly.StartsWith("lib/net10.0/", StringComparison.OrdinalIgnoreCase) && !manifest.EntryAssembly.StartsWith("lib/net10.0\\", StringComparison.OrdinalIgnoreCase))
        ...
        if (manifest.EfCoreVersion is not null && !Version.TryParse(manifest.EfCoreVersion, out _))
