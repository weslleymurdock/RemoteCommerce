...
public sealed class RemoteCommerceLocalizer(IStringLocalizerFactory factory, LocalizationResourceService resourceService) : ILocalizer
{
    ...
    public async Task<string> GetAsync(Type resourceType, string key, params object[] arguments)
    {
        ...
        foreach (var candidate in GetCultureFallbacks(culture))
        {
            ...
            if (entries is not null && entries.TryGetValue(key, out var value)) return Format(value, arguments);
        }
    ...
    private static IEnumerable<string> GetCultureFallbacks(CultureInfo culture)
    {
        ...
        if (!string.Equals(culture.Name, "en-US", StringComparison.OrdinalIgnoreCase)) yield return "en-US";
    }
    private static string BuildCacheKey(string resourceType, string culture) => $"{resourceType}|{culture}";
    private static string Format(string value, object[] arguments) => arguments.Length == 0 ? value : string.Format(CultureInfo.CurrentCulture, value, arguments);
}
