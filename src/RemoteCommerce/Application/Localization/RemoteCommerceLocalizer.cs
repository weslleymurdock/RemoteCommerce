...
public sealed class RemoteCommerceLocalizer(
    IStringLocalizerFactory factory,
    LocalizationResourceService resourceService) : ILocalizer
{
    ...
    public string Get<TResource>(string key, params object[] arguments) =>
        GetAsync<TResource>(key, arguments).GetAwaiter().GetResult();

    /// <inheritdoc />
    public string Get(Type resourceType, string key, params object[] arguments) =>
        GetAsync(resourceType, key, arguments).GetAwaiter().GetResult();
    ...
    public Task<string> GetAsync<TResource>(string key, params object[] arguments) =>
        GetAsync(typeof(TResource), key, arguments);
    ...
    private static string Format(string value, object[] arguments) =>
        arguments.Length == 0 ? value : string.Format(CultureInfo.CurrentCulture, value, arguments);
}
