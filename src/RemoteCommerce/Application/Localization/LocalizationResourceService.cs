...
public sealed class LocalizationResourceService(IDbContextFactory<CommerceDbContext> dbFactory, CommerceDbContext db, IWebHostEnvironment environment) : ILocalizationResourceService
{
    ...
    public async Task<LocalizationResourceImportResult> ImportAsync(Stream content, string culture, string resourceType, Guid? importedByUserId, string actor, CancellationToken cancellationToken = default)
    {
        ...
        var latestVersion = await db.LocalizationResources.Where(x => x.Culture == culture && x.ResourceType == resourceType).Select(x => (int?)x.Version).MaxAsync(cancellationToken) ?? 0;
        ...
        try
        {
            await using (var file = File.Create(path)) await content.CopyToAsync(file, cancellationToken);
            var activeResources = await db.LocalizationResources.Where(x => x.Culture == culture && x.ResourceType == resourceType && x.IsActive).ToListAsync(cancellationToken);
            foreach (var activeResource in activeResources) activeResource.IsActive = false;
            db.LocalizationResources.Add(new LocalizationResource { Culture = culture, ResourceType = resourceType, ContentHash = hash, Version = version, ImportedByUserId = importedByUserId, ImportedAt = DateTime.UtcNow, IsActive = true });
            db.AuditLogs.Add(new AuditLog { UserId = importedByUserId, Actor = string.IsNullOrWhiteSpace(actor) ? "system" : actor, Operation = "localization.resource.import", Resource = resourceType, Result = "Success", Context = $"Culture={culture}; Version={version}; EntryCount={entries.Count}; Hash={hash}", CreatedAt = DateTime.UtcNow });
    ...
    {
        content.Seek(0, SeekOrigin.Begin);
        var settings = new XmlReaderSettings { Async = true, DtdProcessing = DtdProcessing.Prohibit, XmlResolver = null, IgnoreComments = true, IgnoreWhitespace = true };
        ...
        while (await reader.ReadAsync())
        {
            ...
            {
                if (dataReader.NodeType == XmlNodeType.Element && dataReader.Name == "value") { entries[key] = await dataReader.ReadElementContentAsStringAsync(); hasValue = true; break; }
            }
    ...
    {
        content.Seek(0, SeekOrigin.Begin); using var sha = SHA256.Create(); var hash = await sha.ComputeHashAsync(content, cancellationToken); return Convert.ToHexString(hash).ToLowerInvariant();
    }
}
...
{
    /// <summary>Validates, versions, persists, and activates an XML resource.</summary><param name="content">The XML resource stream.</param><param name="culture">The resource culture.</param><param name="resourceType">The resource marker type name.</param><param name="importedByUserId">The importing user identifier.</param><param name="actor">The importing actor display name.</param><param name="cancellationToken">The cancellation token for the operation.</param><returns>The imported resource metadata.</returns>
    ...
    /// <summary>Lists imported localization resource versions.</summary><param name="cancellationToken">The cancellation token for the operation.</param><returns>The known resource versions ordered newest first.</returns>
    Task<IReadOnlyList<LocalizationResourceSummary>> ListAsync(CancellationToken cancellationToken = default);
}
/// <summary>Describes a successfully imported localization resource.</summary><param name="Culture">The resource culture.</param><param name="ResourceType">The resource marker type.</param><param name="Version">The assigned resource version.</param><param name="EntryCount">The number of validated resource entries.</param><param name="ContentHash">The SHA-256 content hash.</param>
...
/// <summary>Describes an imported localization resource version.</summary><param name="Id">The persistence identifier.</param><param name="Culture">The resource culture.</param><param name="ResourceType">The resource marker type.</param><param name="Version">The resource version.</param><param name="ImportedAt">The import timestamp.</param><param name="IsActive">Whether this version is active.</param>
