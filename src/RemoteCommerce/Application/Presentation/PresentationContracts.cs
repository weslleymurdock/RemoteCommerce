...
public sealed class AdminMenuProvider(IEnumerable<IMenuContributor> contributors) : IMenuProvider
{
    /// <inheritdoc />
    public IReadOnlyList<MenuItemDefinition> GetItems(ClaimsPrincipal user) => contributors.SelectMany(x => x.GetItems()).Where(x => x.VisibilityPredicate?.Invoke() != false).Where(x => string.IsNullOrWhiteSpace(x.RequiredPolicy) || user.IsInRole("Administrator")).OrderBy(x => x.Order).ThenBy(x => x.Id).ToArray();
}
