...
public sealed class DefaultThemeProvider : IThemeProvider { /// <inheritdoc /> public ThemeDefinition Current { get; } = new("default", "RemoteCommerce Default", "1.0.0", "RemoteCommerce", ["admin"], ["/app.css"], [], new Dictionary<string, string>(), new Dictionary<string, string>()); }
...
/// <summary>Builds the final administration menu.</summary>
public interface IMenuProvider { /// <summary>Gets visible menu items ordered for presentation.</summary> IReadOnlyList<MenuItemDefinition> GetItems(ClaimsPrincipal user); }
...
public sealed class AdminMenuProvider(IEnumerable<IMenuContributor> contributors, IEnumerable<IRemoteCommercePluginMenuContributor> pluginContributors) : IMenuProvider
{
    ...
    public IReadOnlyList<MenuItemDefinition> GetItems(ClaimsPrincipal user)
    {
        ...
        return core.Concat(plugins).Where(x => x.VisibilityPredicate?.Invoke() != false).Where(x => string.IsNullOrWhiteSpace(x.RequiredPolicy) || user.IsInRole("Administrator")).OrderBy(x => x.Order).ThenBy(x => x.Id).ToArray();
    }
}
