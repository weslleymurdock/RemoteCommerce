...
/// <summary>Resolves the built-in RemoteCommerce presentation theme.</summary>
...
public sealed class AdminMenuProvider : IMenuProvider
{
    ...
    public AdminMenuProvider(
        ...
        IEnumerable<IRemoteCommercePluginMenuContributor> pluginContributors)
    {
        ...
        ClaimsPrincipal user)
    {
        ...
        return core
            ...
            .Where(x =>
                string.IsNullOrWhiteSpace(x.RequiredPolicy) ||
                user.IsInRole("Administrator") ||
                user.HasClaim("permission", x.RequiredPolicy))
            .OrderBy(x => x.Order)
            .ThenBy(x => x.Id)
            .ToArray();
    }
