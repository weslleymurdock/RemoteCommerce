...
public sealed class PluginAdministrationTests
{
    ...
    public async Task DependencyValidation_RejectsCircularDependency()
    {
        var options = CreateOptions();
        await using var db = new CommerceDbContext(options);
        db.PluginInstallations.AddRange(CreateInstallation("plugin-a", "1.0.0", PluginInstallationState.Loaded), CreateInstallation("plugin-b", "1.0.0", PluginInstallationState.Loaded));
    ...
    private static DbContextOptions<CommerceDbContext> CreateOptions()
        => new DbContextOptionsBuilder<CommerceDbContext>().UseInMemoryDatabase(Guid.NewGuid().ToString()).Options;
    ...
    private static async Task<string> CreatePackageAsync(bool includeReadme = true, bool includeLicense = true, string entryAssembly = "lib/net10.0/TestPlugin.dll", string entryType = "Tests.TestPlugin", string minHostVersion = "1.0.0")
    {
        ...
        await using var input = File.OpenRead(typeof(PluginAdministrationTests).Assembly.Location);
