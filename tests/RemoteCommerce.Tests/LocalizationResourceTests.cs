...
public sealed class LocalizationResourceTests
{
    ...
    public async Task ImportAsync_StoresValidResourceAndVersion()
    {
        ...
        try
        {
            ...
            var result = await service.ImportAsync(stream, "pt-BR", "RemoteCommerce.Tests.SharedResource", null, "test");
    ...
    public async Task ImportAsync_RejectsDuplicateKeys()
    {
        ...
        try
        {
            ...
            await Assert.ThrowsAsync<InvalidDataException>(() => service.ImportAsync(stream, "en-US", "SharedResource", null, "test"));
        }
        finally { Directory.Delete(root, recursive: true); }
    }
    ...
    public async Task ImportAsync_RejectsExternalEntities()
    {
        ...
        try
        {
            ...
            await Assert.ThrowsAnyAsync<Exception>(() => service.ImportAsync(stream, "en-US", "SharedResource", null, "test"));
        }
        finally { Directory.Delete(root, recursive: true); }
    }
    ...
    public async Task ImportAsync_RejectsUnsupportedCulture()
    {
        ...
        await Assert.ThrowsAsync<ArgumentException>(() => service.ImportAsync(stream, "fr-FR", "SharedResource", null, "test"));
    }
    ...
    {
        public string EnvironmentName { get; set; } = "Development"; public string ApplicationName { get; set; } = "RemoteCommerce.Tests"; public string WebRootPath { get; set; } = contentRootPath; public IFileProvider WebRootFileProvider { get; set; } = new NullFileProvider(); public string ContentRootPath { get; set; } = contentRootPath; public IFileProvider ContentRootFileProvider { get; set; } = new NullFileProvider();
    }
}
